...
export function useTasks(): UseTasksState {
  ...
  function normalizeTasks(input: any[]): Task[] {
    ...
    return (Array.isArray(input) ? input : []).map((t, idx) => {
      let id = typeof t.id === 'string' && t.id.trim() ? t.id : crypto.randomUUID();
  ...
  useEffect(() => {
  //if we've already fetched, do nothing just return
  ...
  const loadTasks = async () => {
    try {
      ...
      console.error('Task load failed due to unexpected error : ', err);
    }
  };
  ...
  const undoDelete = useCallback(() => {
    ...
    setTasks(prev => prev.some(t => t.id === lastDeleted.id) ? prev : [...prev, lastDeleted]);
    setLastDeleted(null);
  }, [lastDeleted]);
